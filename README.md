...
### 1. **users**
Stores all user accounts (employees and managers)
```sql
- id (PRIMARY KEY)
- email (UNIQUE)
...
- name (UNIQUE)
- description
- annual_quota (default days per year)
...
**Default Leave Types:**
- Vacation Leave (15 days) - #3498db
- Sick Leave (12 days) - #e74c3c
- Personal Leave (5 days) - #9b59b6
- Maternity Leave (90 days) - #e91e63
- Paternity Leave (10 days) - #2196f3
- Casual Leave (7 days) - #f39c12

### 3. **leave_balances**
Tracks leave quotas and usage per user per year
```sql
- id (PRIMARY KEY)
...
- total_days
- used_days
- remaining_days (GENERATED/COMPUTED)
- created_at, updated_at
```
...
### 5. **leave_request_approvals**
Audit trail for approval/rejection actions
```sql
- id (PRIMARY KEY)
- leave_request_id (FOREIGN KEY → leave_requests)
...
{
  "comment": "Staffing shortage during this period"
}
```
...
#### Get Calendar Data
```http
GET /leaves/calendar?month=2&year=2026
Authorization: Bearer {token}
```

### Admin Panel Endpoints (Manager Only)
...
DELETE /admin/leave-types/:id          # Deactivate leave type
```
